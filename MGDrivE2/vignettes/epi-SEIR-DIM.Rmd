...
We start by loading the **MGDrivE2** package, as well as the **MGDrivE** package for access to inheritance cubes, **ggplot2** for graphical analysis, and **Matrix** for sparse matrices used in migration. We will use a simple homing-cube to simulate CRISPR-like gene drive for this example.
...
Before we setup the sims, we need to augment the cube with genotype specific transmission efficiencies, `b` and `c`, the mosquito to human and human to mosquito transmission efficiencies, as well as disease-induced mortality on infectious mosquitoes. For simplification, we assume that transmission from human to mosquito, 
...
With the parameters defined, we use **SEI-SEIR** model specific functions to setup the places and transitions in the Petri Net.
