...
###############################################################################
#   Equilibrium EIR for Imperial Model
###############################################################################
#' Generally, pathogen prevalence is a more accessible metric for users, but
...
###############################################################################
#' Calculate Equilibrium for Imperial Model
#'
#' This function calculates the human and mosquito equilibrium values for the
#' decoupled Imperial model. \strong{Currently this only works in one node.}
#'
#' For mosquito equilibrium, this function calls \code{\link{equilibrium_SEI_Imperial_mosy}} internally.
#'
#' For human equilibrium, this function calls \code{\link{equilibrium_Imperial_decoupled_human}} internally.
#'
...
#'
#' @return a matrix of the equilibrium number of humans in each Imperial stage by
#' age, and immunity. mosquito equilibrium values, and full theta vector
#'
#' @importFrom statmod gauss.quad.prob
#'
#' @export
equilibrium_Imperial_decoupled <- function(age_vector, ft, eir, theta, cube, spn_P) {

  # calculate human equilibrium
  human_eqm <- equilibrium_Imperial_decoupled_human(age_vector, ft, eir, theta)
  ...
  initialCons <- equilibrium_SEI_Imperial_mosy(
    params = theta,
    phi = 0.5,
    log_dd = TRUE,
    spn_P = spn_P,
...
#'
#' \strong{Currently this only works in one node.}
#'
#' @param age_vector age structure of population (see vignette for example)
#' @param ft proportion of population seeking treatment
#' @param EIR desired annual EIR
...
#######################################
# 20221204 JB
#  I think this function is exactly the same as the decoupled one - equilibrium_SEI_decoupled_mosy
#  Potentially better ways of handling the code duplication - I'm not being paid
#  for it right now.
...
#'
equilibrium_SEI_Imperial_mosy <- function(params, node_list="b",NF=NULL,phi=0.5, NH=NULL,log_dd=TRUE,
...
#' Solve Female Distribution with Imperial Model
#'
#' This calculates equilibrium females, I, based on humans and epi info from
#' Imperial model only used in equilibrium_SEI_Imperial_mosy
#'
#' @param params a named list of parameters (see \code{\link{equilibrium_SEI_Imperial_mosy}})
#'
  ...
  with(params,{
    ...
    for(node in 1:nNode){
      ...
      if(FOIv > 0){
        ...
        Qmat <- make_Q_SEI(q=qEIP,n=nEIP,mu=muF,FOI=FOIv)
