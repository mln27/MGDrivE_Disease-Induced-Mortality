...
#' The first site, on an autosomal chromosome, carries gRNA and Cas9 for homing
#' and removal of the SEM site. The second site, on the X chromosome, consists
...
#' one resistance allele associated with SEM. There are 4 possible alleles at the first site:
#' \itemize{
...
#' }
#'
#' There are 4 possible alleles at the second, X-linked target site:
#' \itemize{
...
#' }
#'
#' This provides a total genotype count of 90, however, there are only 60 viable
#' female genotypes and 30 viable male genotypes. Genotypes are written (site 1)(site 1)(site 2)(site 2),
#' where "site 1" is on an autosome and "site 2" is X-linked.
#' Therefore, a genotype of WWHE would be female and completely wild-type at the first locus,
#' and at the second locus a compound heterozygote with the SEM allele and a
#' wild-type equivalent allele.
#'
...
#' @export
cubeSEMtransX_SEMReduct2 <- function(pF=1,
                                     aF=1, bF=1, cF=1,
                                     xF=1,
                                     pM=pF,
                                     aM=aF, bM=bF, cM=cF,
  ...
  genoF <- c('WWXX','GWXX','VWXX','SWXX','GGXX','GVXX','GSXX','VVXX','SVXX','SSXX','WWHX',
             'GWHX','VWHX','SWHX','GGHX','GVHX','GSHX','VVHX','SVHX','SSHX','WWEX','GWEX',
             'VWEX','SWEX','GGEX','GVEX','GSEX','VVEX','SVEX','SSEX','WWHH','GWHH','VWHH',
             'SWHH','GGHH','GVHH','GSHH','VVHH','SVHH','SSHH','WWEH','GWEH','VWEH','SWEH',
             'GGEH','GVEH','GSEH','VVEH','SVEH','SSEH','WWEE','GWEE','VWEE','SWEE','GGEE',
             'GVEE','GSEE','VVEE','SVEE','SSEE')
  genoM <- c('WWXY','GWXY','VWXY','SWXY','GGXY','GVXY','GSXY','VVXY','SVXY','SSXY','WWHY',
             'GWHY','VWHY','SWHY','GGHY','GVHY','GSHY','VVHY','SVHY','SSHY','WWEY','GWEY',
             'VWEY','SWEY','GGEY','GVEY','GSEY','VVEY','SVEY','SSEY')
  ...
  dep1 <- list('W' = list('W' = c('W'=1),
               ...
               'G' = list('W' = c('W'=aDep*bDep*(1-cDep),
                                  'G'=1-aDep,
                                  'V'=aDep*bDep*cDep,
                                  'S'=aDep*(1-bDep)),
                          'G' = c('W'=aDep*bDep*(1-cDep),
                                  'G'=1-aDep,
                                  'V'=aDep*bDep*cDep,
                                  'S'=aDep*(1-bDep)),
                          'V' = c('W'=aDep*bDep*(1-cDep),
                                  'G'=1-aDep,
                                  'V'=aDep*bDep*cDep,
                                  'S'=aDep*(1-bDep)),
                          'S' = c('W'=aDep*bDep*(1-cDep),
                                  'G'=1-aDep,
                                  'V'=aDep*bDep*cDep,
                                  'S'=aDep*(1-bDep)) )
  )
