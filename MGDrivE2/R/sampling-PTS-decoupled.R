...
#' Make Poisson Time-Step (PTS) Sampler for a SPN-Decoupled Model
#'
...
step_PTS_decoupled <- function(S,Sout,haz,sIDX,dt=0.01,maxhaz=1e6,human_ode="SIS"){
  ...
  if(human_ode == "SIS"){
    return(
      function(x0,h0,t0,deltat) {
        ...
        repeat {
          ...
          if(tNow > termt){
            return(
              list("x" = x,"o" = ovec,"h" = human_states)
            )
          }
        } # end loop
      }
    ...
    return(
      function(x0,h0,t0,deltat,human_trace){
        ...
        repeat {
          ...
          # eval meaningful SPN hazards
          h <- haz(M=x,t=tNow,h=human_states,human_trace=human_trace,idx=idx)
          if (any(h > maxhaz)) {
            stop("hazard too large, terminating simulation.\n\ttry reducing dt")
          }
