...
#' Inheritance Cube: ReMEDE (REpeat Mediated Excision of a Drive Element), Full Reduction
#'
...
#' an induceable endonuclease, and is flanked by direct repeats. This version
#' has been reduced, removing all resistance alleles. After that, there are 4 possible alleles:
#' \itemize{
#'  \item W: Wild-type
...
#'
#' Gene-drive and SEM parameters (p*, a*, c*, mmr*) are
...
cubeSEMreduct1 <- function(pF=1,
  ...
  # # Testing Probs
  # testVec <- runif(n = 9, min = 0, max = 1)
  #
  # mmrF <- testVec[1]; mmrM <- testVec[2]
  # pF <- testVec[3];
  # aF <- testVec[4]; cF <- testVec[5];
  # pM <- testVec[6];
  # aM <- testVec[7]; cM <- testVec[8];
  # pDep <- testVec[9];
  ...
  genotypes <- c('WW','GW','VW','HW','GG','GV','GH','VV','HV','HH')
  ...
  for(fi in 1:numGen){
    ...
    gdScoreF <- semScoreF || any("G" == fSplit)
    ...
    for(mi in 1:numGen){
      ...
      gdScoreM <- semScoreM || any("G" == mSplit)
