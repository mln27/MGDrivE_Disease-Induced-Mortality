...
#' This is based on the work of Le Menach et al. (2007) and Griffin et . (2010).
...
add_interventions <- function(params, IRS_cov, LLIN_cov) {
  ...
  params_vc <- params # create new vector to hold parameters under vector control
  coverage <- generate_coverage_parameters(IRS_cov, LLIN_cov)

  # feeding rate modifications
  feeding_params <- modify_feeding_rate(IRS_cov, LLIN_cov, params_vc, coverage)
  ...
  mortality_params <- modify_adult_mortality(IRS_cov, LLIN_cov, params_vc, coverage)
...
modify_feeding_rate <- function(IRS_cov, LLIN_cov, params, coverage) {
...
modify_adult_mortality <- function(IRS_cov, LLIN_cov, params, coverage) {
